"""测试 cross_lingual 模式（纯新内容，无 prompt 文本干扰）。"""
import sys
import time
...
def main():
    ...
    text = "今天天气真好，适合出去散步。"
    for output in cv.inference_cross_lingual(
        tts_text=text,
        ...
        with wave.open(r"D:\dsh\voice-assistant\output\cosyvoice_cross.wav", "wb") as w:
            w.setnchannels(1)
            w.setsampwidth(2)
            w.setframerate(24000)
            w.writeframes(pcm.tobytes())
        print("[tts] saved cross.wav")

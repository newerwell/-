...
def main():
    ...
    for t in gen:
        if hasattr(t, "item"):
            ...
            tokens.append(int(t))
    print(f"[diag] tokens: {len(tokens)}")
    ...
    print(f"[diag] speech: {sp.shape[0]/24000:.1f}s RMS={np.sqrt(np.mean(sp**2)):.3f}")

    pcm = (np.clip(sp, -1, 1) * 32767).astype(np.int16)
    with wave.open(r"D:\dsh\voice-assistant\output\cosyvoice_greedy.wav", "wb") as w:
        w.setnchannels(1)
        w.setsampwidth(2)
        w.setframerate(24000)
        w.writeframes(pcm.tobytes())
    print("[diag] saved greedy.wav")

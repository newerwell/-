...
def main():
    ...
    model_input = cv.frontend.frontend_zero_shot(
        "今天天气真好。",
        "欢迎大家来体验达摩院",  # 短 prompt 文本
        r"D:\dsh\voice-assistant\output\prompt_1s.wav",
        24000, "",
    )
    print(f"[diag] prompt token: {model_input['llm_prompt_speech_token'].shape}")
    ...
    with wave.open(r"D:\dsh\voice-assistant\output\cosyvoice_greedy_1s.wav", "wb") as w:
        w.setnchannels(1)
        w.setsampwidth(2)
        w.setframerate(24000)
        w.writeframes(pcm.tobytes())
    print("[diag] saved greedy_1s.wav")
